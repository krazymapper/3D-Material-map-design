...
function init() {
	...
	listeChamps[0] = "prod_lait";
	listeChamps[1] = "fruit_leg";
	listeChamps[2] = "viande_cha";
	listeChamps[3] = "poisson";
	listeChamps[4] = "patis_boul";
	listeChamps[5] = "plat_cuis";
	listeChamps[6] = "epice";
...
function afficheCarte(error, fond, lieux_aplat, lieux, lieux_centroides) {
	...
	// calcule les décalages en X
	//offset_epice = calculOffset(lieux_centroides, "epice");
	//console.log(offset_epice);
	// affiche les centroïdes
	afficheCentroides(lieux_centroides, "prod_lait", "#F4F4F4", "Produits laitiers")
	appendCentroides(lieux_centroides, "fruit_leg", "#10F42E", "Fruits, légumes et champignons")
	appendCentroides(lieux_centroides, "viande_cha", "#D44652", "Viandes, charcuteries, salaisons et fumaisons")
	appendCentroides(lieux_centroides, "poisson", "#47A9F9", "Poissons et fruits de mer")
	appendCentroides(lieux_centroides, "patis_boul", "#FFA63F", "Pâtisserie, confiserie, boulangerie")
	appendCentroides(lieux_centroides, "plat_cuis", "#6530AA", "Plat cuisiné")
	appendCentroides(lieux_centroides, "epice", "#4C1000", "Epice")
...
function appendCentroides(centroides, champ, couleur, intitule) {
	svg.append("g").selectAll('.lieux_centroides')
		...
		.attr("transform", function(d) {
			offset = 0
			for (i = 0; i < listeChamps.indexOf(champ); i++) {
				if (Boolean(Number(d.properties[listeChamps[i]])) == true) {
					offset += 5
				}
			}	
