...
function init() {
	...
	// titre
	var titre = "<p>Répartition des produits laitiers par source</p>";
...
function afficheCarte(error, fond, lieux_aplat, lieux, lieux_centroides) {
	...
	svg.selectAll('.lieux_centroides')
		...
		.attr("d", d3.svg.symbol()
			.type("circle")
			.size(function (d) {return Math.pow(Math.sqrt(d.properties.prod_lait/Math.PI)*20, 2); }))
		...
		.on('mousemove', function (d) {
			...
			tooltip.classed('hidden', false)
				...
				.html(d.properties.lieu + " : <br>" + d.properties.prod_lait + " produit(s) laitier(s)");
		 })
